...
def load_llm(provider_name: str, model: str | None = None, **kwargs: Any) -> LLM:
    ...
    if provider_name == "OpenAILike":
    ...
    elif provider_name == "CustomAPI":
        from droidrun.agent.utils.custom_llm import SimpleCustomLLM
        
        # Extract specific keys if present in kwargs
        api_url = kwargs.pop("api_url", None)
        if not api_url:
            # Fallback to base_url or api_base if user uses those names
            api_url = kwargs.pop("base_url", kwargs.pop("api_base", None))
            
        if not api_url:
            raise ValueError("CustomAPI requires 'api_url' in configuration.")
            
        return SimpleCustomLLM(
            api_url=api_url,
            model_name=model or "custom-model",
            prompt_key=kwargs.pop("prompt_key", "prompt"),
            response_key=kwargs.pop("response_key", "text"),
            **kwargs
        )
    elif provider_name == "GroqSDK":
        from droidrun.agent.utils.custom_llm import GroqSDKLLM
        import os
        GROQ_API_KEY = os.getenv("GROQ_API_KEY") or ""
        api_key = kwargs.pop("api_key", GROQ_API_KEY)
        if not api_key:
            raise ValueError("GroqSDK requires 'api_key' in configuration or GROQ_API_KEY env var.")
            
        # Fix: Pop 'model' from kwargs because load_llm already added it there
        ...
        return GroqSDKLLM(
            ...
            max_tokens=kwargs.pop("max_completion_tokens", 8192),
